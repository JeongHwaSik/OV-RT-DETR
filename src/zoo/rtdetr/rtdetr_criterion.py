    ...
    def forward(self, outputs, targets):
        ...
        Parameters:
             outputs: dict of tensors, see the output specification of the model for the format
                      e.g. outputs.keys() >>> dict_keys(['pred_logits', 'pred_boxes', 'aux_outputs', 'dn_aux_outputs', 'dn_meta'])
             targets: list of dicts, such that len(targets) == batch_size.
                      The expected keys in each dict depends on the losses applied, see each loss' doc
                      e.g. targets[0].keys() >>> dict_keys(['boxes', 'labels', 'image_id', 'area', 'iscrowd', 'orig_size', 'size'])
        """
        outputs_without_aux = {k: v for k, v in outputs.items() if 'aux' not in k}

        # Retrieve the matching between the outputs of the last layer and the targets
        indices = self.matcher(outputs_without_aux, targets) # len(indices) = bs
